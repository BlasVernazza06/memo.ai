...
interface EmptySearchWorkspaceSecProps {
  setSearchQuery: (query: string) => void;
}

export default function EmptySearchWorkspaceSec({
  setSearchQuery,
}: EmptySearchWorkspaceSecProps) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
      <div className="w-24 h-24 bg-muted rounded-4xl flex items-center justify-center mb-4 border border-border shadow-inner group transition-all hover:scale-105">
        <Search className="w-12 h-12 text-muted-foreground group-hover:text-primary transition-colors" />
      </div>
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground max-w-xs font-bold uppercase text-[10px] tracking-widest leading-relaxed">
          Prueba con otra palabra clave para filtrar tus workspaces.
        </p>
      </div>
      <Button
        variant="outline"
