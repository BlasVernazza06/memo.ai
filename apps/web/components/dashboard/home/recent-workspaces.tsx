      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {recent.map((ws, idx) => (
          <Link key={ws.id} href={`/dashboard/workspaces/${ws.id}`}>
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl bg-linear-to-br ${ws.color || 'from-primary to-blue-600'} opacity-90 shrink-0 flex items-center justify-center text-white shadow-sm font-black`}
              >
                {ws.name.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-foreground truncate group-hover:text-primary transition-colors text-sm italic">
                  {ws.name}
                </h3>
