    ...
    const questions = [
        {
            ...
            correctIndex: 1,
            explanation: "El esqueleto axial incluye el cráneo, la columna vertebral y la caja torácica (donde está el esternón)."
        },
        {
            id: 2,
            question: "¿Qué tipo de articulación permite el mayor rango de movimiento?",
            options: ["Sinartrosis", "Anfiartrosis", "Diartrosis (Sinovial)", "Suturas"],
            correctIndex: 2,
            explanation: "Las diartrosis o articulaciones sinoviales, como la del hombro, permiten movimientos amplios."
        },
        {
            id: 3,
            question: "La médula ósea roja es responsable de:",
            options: ["Almacenar grasa", "Producir células sanguíneas", "Proteger nervios", "Absorber calcio"],
            correctIndex: 1,
            explanation: "La hematopoyesis (producción de células sanguíneas) ocurre en la médula ósea roja."
        }
    ];
            ...
            <div className="min-h-[80vh] flex flex-col items-center justify-center max-w-2xl mx-auto space-y-8 text-center p-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <motion.div 
                    ...
                    className={`w-32 h-32 rounded-full flex items-center justify-center mb-4 mx-auto shadow-2xl ${
                        percentage >= 70 ? 'bg-emerald-100 text-emerald-500 shadow-emerald-500/20' : 'bg-orange-100 text-orange-500 shadow-orange-500/20'
                    }`}
                >
                ...
                <div className="bg-white border border-slate-100 p-8 rounded-[2rem] w-full shadow-sm">
                    <div className="flex justify-center items-end gap-2 mb-2">
                        ...
                        <span className="text-xl font-bold text-slate-400 mb-2">Aciertos</span>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full pt-6">
                    <Button onClick={restartQuiz} variant="outline" className="flex-1 h-14 rounded-2xl border-slate-200 text-slate-600 font-bold hover:bg-slate-50 hover:text-slate-900 transition-colors">
            ...
            <div className="space-y-8">
                ...
                <div className="grid gap-3">
                    {currentQuestion.options.map((option, index) => {
                        ...
                        let cardClass = "bg-white border-2 border-slate-100 hover:border-slate-200 hover:bg-slate-50 text-slate-600";
                        if (showResult) {
                            if (isCorrect) cardClass = "bg-emerald-50 border-emerald-200 text-emerald-800 ring-2 ring-emerald-100 shadow-lg shadow-emerald-500/10";
                            else if (isSelected && !isCorrect) cardClass = "bg-red-50 border-red-200 text-red-800 opacity-80 ring-2 ring-red-100";
                            else cardClass = "bg-slate-50 border-slate-100 text-slate-400 opacity-50";
                        } else if (isSelected) {
                            cardClass = "bg-primary/5 border-primary text-primary ring-2 ring-primary/20";
                        }
                        ...
                        return (
                            ...
                            >
                                ...
                                <AnimatePresence>
                                    {showResult && isCorrect && (
                                        <motion.div initial={{ scale: 0, rotate: -45 }} animate={{ scale: 1, rotate: 0 }} className="bg-emerald-100 p-1 rounded-full">
                                            <Check className="w-5 h-5 text-emerald-600 shrink-0" />
                                        </motion.div>
                                    )}
                                    {showResult && isSelected && !isCorrect && (
                                        <motion.div initial={{ scale: 0, rotate: 45 }} animate={{ scale: 1, rotate: 0 }} className="bg-red-100 p-1 rounded-full">
                                            <X className="w-5 h-5 text-red-500 shrink-0" />
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                                {!showResult && isSelected && <div className="w-4 h-4 rounded-full bg-primary shrink-0" />}
                                {!showResult && !isSelected && <div className="w-4 h-4 rounded-full border-2 border-slate-200 group-hover:border-slate-300 shrink-0" />}
                            </motion.button>
                        );
                    })}
                </div>
                    ...
                    {isAnswered ? (
                        ...
                        >
                            ...
                            <div className="flex justify-end pt-2">
                                <Button onClick={handleNextQuestion} className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold px-8 py-6 shadow-lg shadow-blue-500/20 transition-all active:scale-95">
