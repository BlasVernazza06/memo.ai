...
type TabId = 'profile' | 'billing' | 'security' | 'notifications' | 'appearance';
        ...
        switch (activeTab) {
                    ...
                    >
                                ...
                                <div className="relative group">
                                    <div className="w-32 h-32 rounded-3xl bg-linear-to-br from-primary to-blue-600 p-1 shadow-2xl shadow-primary/20">
                                        <div className="w-full h-full rounded-[1.25rem] bg-white flex items-center justify-center overflow-hidden">
                                            <span className="text-4xl font-black text-primary">BV</span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex-1 space-y-6 w-full">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            ...
                                            <Input defaultValue="Blas Vernazza" className="rounded-xl border-slate-200 bg-slate-50/50 h-11 focus:ring-primary/10" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Email</Label>
                                            <Input defaultValue="blas@memo.ai" disabled className="rounded-xl border-slate-200 bg-slate-100 h-11 text-slate-500 cursor-not-allowed" />
                                        </div>
                                    </div>
                        ...
                        <div className="p-8 border-2 border-dashed border-red-100 rounded-4xl flex flex-col md:flex-row justify-between items-center gap-6 bg-red-50/10">
                            <div className="space-y-1">
                                ...
                                <p className="text-sm text-slate-400 font-medium">Esta acción es irreversible y borrará todos tus datos.</p>
                            </div>
                            <Button variant="ghost" className="text-red-400 hover:text-red-500 hover:bg-red-50 font-bold px-6">
                    ...
                    >
                        <div className="relative overflow-hidden bg-slate-900 rounded-4xl p-8 text-white shadow-xl">
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        <h3 className="text-2xl font-bold italic tracking-tight text-primary">Memo Unlimited</h3>
                                        <p className="text-slate-400 text-sm font-medium">Próximo cobro: $9.99 el 12 de Marzo, 2026</p>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                ...
                                <div key={i} className="bg-white border border-slate-100 rounded-3xl p-6 flex items-center gap-4">
                    ...
                    >
                        <div className="bg-white border border-slate-200/60 rounded-4xl p-8 shadow-sm space-y-8">
                            <div>
                                ...
                                <p className="text-sm text-slate-400 font-medium">Asegúrate de usar una contraseña segura y única.</p>
                            </div>
                            ...
                            <div className="space-y-4 max-w-md">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <Input type="password" placeholder="••••••••" className="rounded-xl bg-slate-50/50 h-11 pl-10" />
                                ...
                                <Button className="bg-primary font-bold rounded-xl px-8 h-11 w-full md:w-auto">Actualizar Contraseña</Button>
                            </div>
                        </div>
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400 font-medium">Añade una capa extra de seguridad a tu cuenta.</p>
                                    </div>
                                </div>
            ...
            case 'notifications':
                return (
                    ...
                    >
                        <div className="bg-white border border-slate-200/60 rounded-4xl p-8 shadow-sm">
                            <h3 className="text-xl font-bold mb-6">Canales de Notificación</h3>
                            <div className="space-y-6">
                                {[
                                    { title: "Nuevas funcionalidades", desc: "Recibe actualizaciones sobre las últimas mejoras de la IA.", icon: Sparkles, active: true },
                                    { title: "Newsletter semanal", desc: "Un resumen de tus avances y estadísticas de estudio.", icon: Mail, active: false },
                                    { title: "Alertas de seguridad", desc: "Avisos sobre inicios de sesión desconocidos.", icon: Shield, active: true },
                                ].map((notif, i) => (
                                    <div key={i} className="flex items-center justify-between py-4 border-b border-slate-50 last:border-0 group">
                                        ...
                                        <div className={`w-12 h-6 rounded-full relative cursor-pointer p-1 transition-colors ${notif.active ? 'bg-primary' : 'bg-slate-200'}`}>
                                            <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${notif.active ? 'translate-x-6' : 'translate-x-0'}`} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    >
                        <div className="bg-white border border-slate-200/60 rounded-4xl p-8 shadow-sm">
                            ...
                            <div className="space-y-8">
                                <div className="space-y-4">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Tema del Sistema</Label>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {[
                                            { id: 'light', label: "Claro", icon: Sun, active: true },
                                            { id: 'dark', label: "Oscuro", icon: Moon, active: false },
                                            { id: 'system', label: "Sistema", icon: Laptop, active: false },
                                        ].map((theme) => (
                                            <div 
                                                key={theme.id}
                                            ...
                                            >
                                                <theme.icon className={`w-6 h-6 ${theme.active ? 'text-primary' : 'text-slate-400'}`} />
                                                <span className={`text-sm font-bold ${theme.active ? 'text-primary' : 'text-slate-500'}`}>{theme.label}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 border-t border-slate-50 pt-8">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="font-bold text-slate-900">Animaciones reducidas</h4>
                                            <p className="text-xs text-slate-400 font-medium">Suaviza las transiciones si prefieres menos movimiento.</p>
                                        </div>
                                        <div className="w-12 h-6 bg-slate-200 rounded-full relative cursor-pointer p-1">
                                            <div className="w-4 h-4 bg-white rounded-full shadow-sm" />
                                        </div>
